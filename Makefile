...
# openstack.org/ovs-operator-bundle:$VERSION and openstack.org/ovs-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/$(USER)/ovs-operator
...
DEFAULT_IMG ?= quay.io/openstack-k8s-operators/ovs-operator:latest
IMG ?= $(DEFAULT_IMG)
...
# Please ensure the ovs-controller-manager deployment and
...
# $oc delete -n openstack validatingwebhookconfiguration/vovs.kb.io
